...
import { ContentStatus, TextDocument, Cortex } from "./index";
...
test(
  ...
  async () => {
    ...
    const catalogName = `catalog-sdk-content-test-${suffix}`;
    ...
    // create
    let catalog;
    let cortex: Cortex | undefined = undefined;
    try {
      ...
      cortex = await testClient.configureCortex(
        `cortex-sdk-content-test-${suffix}`,
        {
      ...
      // edit content
      const editedContent = await content.edit({
        title: "foo",
        content: "bar",
      });
...
test("test streaming content", { timeout: 180000 }, async () => {
  const suffix = Date.now();
  const catalogName = `catalog-sdk-streaming-content-test-${suffix}`;
  ...
  try {
    ...
    cortex = await testClient.configureCortex(
      `cortex-sdk-streaming-content-test-${suffix}`,
      {
        catalogs: [catalog.name],
        friendlyName: "Cortex AI",
        instructions: [
...
test(`test content status and publishing`, { timeout: 180000 }, async () => {
  ...
  try {
    cortex = await testClient.configureCortex(
      `cortex-sdk-content-status-test-${Date.now()}`,
      {
        friendlyName: "Cortex AI",
        instructions: [
          "answer questions about the cortex click AI GTM platform",
