...
export class Chat {
    ...
    static async create(opts: CreateChatOptsStreaming): Promise<StreamingChatResult>;
    static async create(opts: CreateChatOptsSync | CreateChatOptsStreaming): Promise<Chat | StreamingChatResult> {
    ...
    private static async createContentStreaming(opts: CreateChatOptsStreaming): Promise<StreamingChatResult> {
        const { client, cortex, message } = opts;
        const res = await client.POST(`/chats`, { cortex: cortex.name, message, stream: true });
    ...
    private async respondChatSync(opts: RespondChatOptsSync): Promise<string> {
        ...
        const res = await this.apiClient.POST(`/chats/${this.id}`, { message, cortex });
        const body = await res.json();
        this.messages.push(
            {
                role: "user",
    ...
    private async respondChatStreaming(opts: RespondChatOptsStreaming): Promise<StreamingChatResult> {
        const { message, cortex } = opts;
        const res = await this.apiClient.POST(`/chats/${this.id}`, { message, cortex, stream: true });
        const reader = res.body!.getReader();
        const decoder = new TextDecoder('utf-8');
...
function isCreateChatOptsSync(opts: CreateChatOptsSync | CreateChatOptsStreaming): opts is CreateChatOptsSync {
    return opts.stream === false || opts.stream === undefined;
}

function isRespondChatOptsSync(opts: RespondChatOptsSync | RespondChatOptsStreaming): opts is RespondChatOptsSync {
    return opts.stream === false || opts.stream === undefined;
}