...
import { DocumentBatch, DocumentInput, FileDocument, Document, DocumentListItem } from "./document.js";
import * as fs from 'node:fs';
import { JSONIndexer, JSONIndexerOpts } from "./indexers/json-indexer.js";
import { DirectoryIndexer, DirectoryIndexerOpts } from "./indexers/directory-indexer.js";
import { TSVIndexer, TSVIndexerOpts } from "./indexers/tsv-indexer.js";
import { ShopifyIndexer, ShopifyIndexerOpts } from "./indexers/shopify-indexer.js";
...
export class Catalog {
    ...
    private constructor(readonly config: CatalogConfig, private apiClient: CortexApiClient, readonly name: string) {}
    ...
    static async configure(apiClient: CortexApiClient, name: string, config: CatalogConfig): Promise<Catalog> {
    ...
    static async list(apiClient: CortexApiClient): Promise<CatalogListResult[]> {
        ...
        for(const catalog of body.catalogs) {
            result.push({
                ...
                Catalog: () => { return Catalog.get(apiClient, catalog.name) }
            })
        } 
    ...
    public async upsertDocuments(batch: DocumentBatch) {
        ...
        if([hasText, hasJson, hasFile].filter(v => v).length > 1) {
            throw new Error(`cannot mix file, text, and json content in batch upsert. all documents in batch must have the same contentType.`);
        }
        ...
        if(blobs.length === 0) {
            ...
            res = await this.apiClient.POSTForm(`/catalogs/${this.name}/documents`, fd);
        }
    ...
    async listDocuments(paginationOpts?: DocumentPaginationOpts): Promise<DocumentListResult> {
        ...
        this.checkDeleted();
        const res = await this.apiClient.GET(`/catalogs/${this.name}/documents?page=${page}&pageSize=${pageSize}`);
        if (res.status !== 200) {
