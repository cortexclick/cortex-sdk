...
export class JSONIndexer {
    ...
    public static defaultGetId(document: any): string {
        const id = JSONIndexer.findFirstMatchingProperty(document, "id", "Id", "ID", "documentId", "documentID", "DocumentId", "DocumentID");
        if (!id) {
            throw new Error("All documents must contain an 'id' or 'documentId' field, or you must provide opts.getId when using JSONIndexer.");
        }
